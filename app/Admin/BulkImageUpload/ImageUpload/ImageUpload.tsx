...
import { bulkImageUpload, createSelectedImage, getFileKey } from "@/lib/images/uploads";
...
export default function ImageUpload() {
    ...
    function addFiles(files: FileList | File[]) {
        ...
        setSelectedImages((currentImages) => {
            const existingKeys = new Set(currentImages.map((image) => getFileKey(image.file)));

            const newImages = imageFiles.filter((file) => !existingKeys.has(getFileKey(file))).map(createSelectedImage);;
    ...
    async function handleSave() {
        if (selectedImages.length === 0) { setError("Please select at least one image."); return; }
