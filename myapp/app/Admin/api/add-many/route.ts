  ...
  try {
    ...
    switch (type) {
        case "CARD": {
            ...
            if (!item.cardId) {
                throw new Error(`cardId is required for CARD item ${item.uploadId}`);
            }
        ...
        case "PRINT":
            const printItems = items.map((item, index) =>{
                if (!item.printId){
                    throw new Error(`PrintId is required for PRINT item ${item.uploadId}`);
                }
        ...
        case "NOTEPAD":
            const notePadItems = items.map((item, index) =>{
                if (!item.notePadName ){
                    throw new Error(`notePadName is required for NOTEPAD item ${item.uploadId}`);
                }
    ...
    //   case "GIFT":
    //     created = await createGifts(
    //       items.map((item, index) => ({
    //         ...baseItems[index],
    //         giftNumber: item.giftNumber ?? undefined,
    //         giftType: item.giftType ?? undefined,
    //       }))
    //     );
    //     break;
